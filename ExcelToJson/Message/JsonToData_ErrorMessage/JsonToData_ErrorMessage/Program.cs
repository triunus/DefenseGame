...
namespace ExcelToJsonConverter
{
                ...
                foreach (var worksheet in package.Workbook.Worksheets)
                {
                    foreach (var table in worksheet.Tables)
                    {
                        ...
                        for (int rowNumber = 1; rowNumber < rowCount; ++rowNumber)
                        {
                            ...
                            sheetTableData.Add(tableRowData);
                        }

                        workbookData.Add(table.Name, sheetTableData);
                    }
                }
            }
    ...
    public class Program
    {
        public static void Main()
        {
           ...
           Dictionary<string, List<Dictionary<string, object>>> allExcelData = new Dictionary<string, List<Dictionary<string, object>>>();
            ...
            foreach (string excelFile in Directory.GetFiles(baseInfo["ExcelFilesPath"], "*.xlsx"))
            {
                ...
                allExcelData = excelToJson.ReadExcelFile(excelFile);
            }
